...
export class ListarReporteGeneralSupervisorComponent {
  ...
  getVacaciones(): void {
    this.crudService
      .ObtenerVacacionesPersonalBySupervisor(this.user, this.passw)
      .subscribe((respuesta) => {
        ...
        this.btnRechazar = this.VacacionesBySupervisor.some(
          (row) => row.status === 'pendiente'
        );
        this.btnAceptar = this.VacacionesBySupervisor.some(
          (row) => row.status === 'pendiente'
        );

        this.totalPages = Math.ceil(
          this.VacacionesBySupervisor.length / this.pageSize
        );
        this.changePage(1);
      });
  }
  ...
  filterVacaciones(): void {
    ...
    const searchKeyword = this.formularioDeVacacion.get('searchKeyword')
      ?.value;

    if (searchKeyword.trim() !== '') {
      this.VacacionesBySupervisor = this.VacacionesBySupervisor.filter(
        (row) =>
  ...
  AceptarRegistro(id: any, iControl: any) {
    ...
    aux = 'Aprobado';
    this.crudService
      .AceptarSolicitudVacacionBySupervisor(id)
      .subscribe((respuesta) => {
        console.log('respuesta: ', respuesta);
        this.reloadMenuComponent();
        this.getVacaciones();
        this.crudService
          .EnviarCorreoCambioEstadoSolicitud(id)
          .subscribe((respuesta15) => {
  ...
  RechazarRegistro(id: any, iControl: any) {
    ...
    if (!this.formularioDeVacacion.get('motivo')?.value) {
      ...
      this.crudService
        .RechazarSolicitudVacacionBySupervisor(id, aux)
        .subscribe((respuesta) => {
          console.log('respuesta: ', respuesta);
          this.reloadMenuComponent();
          this.getVacaciones();
